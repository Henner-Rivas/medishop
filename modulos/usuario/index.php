                    ...
                    <form id="formulario" method="post">
                        <input type="hidden" name="id_usuario" id="id_usuario"/>
                           ...
                           <div class="form-group row">
                            <label for="idpersona" class="col-sm-3 col-form-label">Persona</label>
                            <div class="col-sm-9">
                                <select class="form-control " id="idpersona" name="idpersona">
                                    <option value="">(Seleccionar el empleado)</option>
                                    <?php
                                    $sql1 = "SELECT persona.idpersona,
concat_ws('  ', persona.nombre  ,persona.apellidos  ,persona.identificacion )  as persona 
...
<script>
      ...
      function mover_pagina(pagina) {
        ...
        $("#listado").load("?modulo=usuario&accion=listar",parametros);
    }
    ...
    function modificar(id) {
       ...
       $("#div-tabla").hide();
        ...
        var p = "id=" + id;
        $.get("?modulo=usuario&accion=asignar", p, function(respuesta) {
  ...
  function eliminar(id) {
        ...
        if (confirm("¿Desea eliminar el registro?")) {
            var p = "id_usuario=" + id;
            $.post("?modulo=usuario&accion=eliminar", p, function(respuesta) {
                try {
                    var r = jQuery.parseJSON(respuesta);
    ...
    $("#btn-agregar").click(function() {
        ...
        $.post("?modulo=usuario&accion=agregar", parametros, function(respuesta) {
            $("#div-pb").hide();
            $("#div-btn").show();
    ...
    $("#btn-modificar").click(function() {
        if (confirm("¿Desea modificar el registro?")) {
            ...
            $("#div-btn").hide();
            var parametros = $("#formulario").serialize();
            $.post("?modulo=usuario&accion=modificar", parametros, function(respuesta) {
                $("#div-pb").hide();
                $("#div-btn").show();
