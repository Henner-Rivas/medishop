            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form id="formulario" method="post">
                        <input type="hidden" name="idproducto_salida" id="idproducto_salida"/>
                        ...
                        <div class="form-group row">
                            <label for="idsalida" class="col-sm-3 col-form-label">Salida de productos</label>
                            <div class="col-sm-9">
                                <select class="form-control " id="idsalida" name="idsalida">
                                    <option value="">(Seleccionar la fecha de salida)</option>
                                    <?php
                                    $sql1 = "SELECT   
                                       salida.idsalida,
                                       fecha as fecha_salida FROM salida
                                   ";
                                    $rs1 = mysqli_query($conexion, $sql1);
                                    while ($rw1 = mysqli_fetch_assoc($rs1)) {

                                        echo "<option value='$rw1[idsalida]'>$rw1[fecha_salida]</option>";
                                    }
                                    ?>
                                </select>
                            </div>
...
<script>
      ...
      function mover_pagina(pagina) {
        ...
        $("#listado").load("?modulo=producto_salida&accion=listar",parametros);
    }
    ...
    function modificar(id) {
       ...
       $("#div-tabla").hide();
        ...
        $("#btn-agregar").hide();
        $("#btn-modificar").show();
            $(".entrada").text("Editar la salida del producto");


        //Limpiar el formulario
        $("#formulario").trigger("reset");
        var p = "id=" + id;
        $.get("?modulo=producto_salida&accion=asignar", p, function(respuesta) {
            ...
            $("#idproducto_salida").val(r.idproducto_salida);
            $("#idproducto").val(r.idproducto);
            $("#idsalida").val(r.idsalida);
    ...
    function eliminar(id) {
        ...
        if (confirm("¿Desea eliminar el registro?")) {
            var p = "idproducto_salida=" + id;
            $.post("?modulo=producto_salida&accion=eliminar", p, function(respuesta) {
                try {
                    var r = jQuery.parseJSON(respuesta);
    ...
    $("#btn-mostrar-formulario-agregar").click(function() {
        ...
        $("#btn-agregar").show();
        $("#btn-modificar").hide();
                    $(".entrada").text("Agregar la salida del producto");
    ...
    $("#btn-agregar").click(function() {
        ...
        $.post("?modulo=producto_salida&accion=agregar", parametros, function(respuesta) {
            $("#div-pb").hide();
            $("#div-btn").show();
    ...
    $("#btn-modificar").click(function() {
        if (confirm("¿Desea modificar el registro?")) {
            ...
            $("#div-btn").hide();
            var parametros = $("#formulario").serialize();
            $.post("?modulo=producto_salida&accion=modificar", parametros, function(respuesta) {
                $("#div-pb").hide();
                $("#div-btn").show();
